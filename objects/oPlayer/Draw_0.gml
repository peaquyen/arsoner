...
if (invulnerable !=  0) && ((invulnerable mod 8/*flicker*/ < 2) == 0) && (flash == 0)
{	
	//skip draw	
}
else
...
//Hookshot (after player) => above player
if ((state == PlayerStateHook) && (image_index == 3)) DrawHookChain();

function DrawHookChain(){
	var _originX = floor(x);
	var _originY = floor(y)-7; //magic number!
	...
	draw_sprite(sHookBlade,image_index,_originX+hookX,_originY+hookY); //hookX,Y are vector
